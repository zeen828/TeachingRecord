...
# 額外指定panel(如果要把全部面板都透過目路區分需要參考下面設定)
php artisan make:filament-resource User --panel=temple --generate --view
```

#### Admin後台面板設定目路區分開其他面板的檔案
```php
...
### ENV的設定跟config上的要對一下，他有分SERVER端跟
```bash
